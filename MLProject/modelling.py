...
def find_data_file(filename):
    """Cari data file di beberapa lokasi."""
    ...
    for dir_path in [script_dir, os.path.dirname(script_dir), os.getcwd()]:
        ...
        if os.path.exists(full_path):
            return full_path
        # Coba dengan basename juga
        basename = os.path.basename(filename)
        full_path2 = os.path.join(dir_path, basename)
...
def main():
    ...
    # === FIX: Jangan set experiment atau start_run! ===
    # MLflow run sudah dibuat oleh 'mlflow run' command
    # Kita langsung log ke run yang aktif
    ...
    # Log params (langsung, tanpa start_run)
